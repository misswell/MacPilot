...
- It appears the instant you click the Dock icon, with the app icons filling in right after. Once closed, the helper stays warm for five minutes (and macOS may reclaim it sooner under memory pressure), so clicking the icon again reopens it in roughly 10 ms instead of paying the 0.2 s-plus process launch and resolution path again.
- The popover always opens next to the Dock icon you clicked: centred on that icon along the Dock, and flush against the Dock's inner edge on the other axis (left, bottom, and right Docks alike) — it no longer follows the pointer's landing point.
- Group icons follow the system appearance: in dark mode they switch to a darkened background with a softer accent, and the glyph stays legible on that darker fill. The helper's Dock icon keeps the light variant — an `.icns` cannot carry appearance variants, third-party Dock icons do not change with the system appearance either, and following whatever appearance happened to be active at generation time would leave the icon wrong after the user switched.
