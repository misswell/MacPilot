...
- The popover header is just the group name plus a settings gear (shortcut `⌘,`), and the footer keeps a single line of guidance; it has no title bar, no second Dock icon, and no normal window chrome; it closes on Escape or an outside click and supports arrow-key navigation and Retina.
