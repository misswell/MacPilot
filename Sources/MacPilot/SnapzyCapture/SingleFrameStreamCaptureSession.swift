...
final class SingleFrameStreamCaptureSession: NSObject, @unchecked Sendable {
  ...
  @discardableResult
  nonisolated func arm(continuation: CheckedContinuation<CGImage, Error>, timeout: TimeInterval) -> Bool {
    lock.lock()
    if finished {
      lock.unlock()
      continuation.resume(throwing: ScreenCaptureError.captureFailed(
        AppText.value("scCaptureAlreadyFinished", language: .system)
      ))
      return false
    }
    ...
    // Cancellation can arrive before the continuation closure is scheduled.
    // Preserve that state so the caller cannot remain suspended forever.
    if wasCancelled {
      finish(.failure(ScreenCaptureError.captureFailed(
        AppText.value("scCaptureCancelled", language: .system)
      )))
      return false
    }
    ...
    let timeoutTask = Task { [weak self] in
      ...
      self?.finish(.failure(ScreenCaptureError.captureFailed(AppText.value("scCaptureTimedOut", language: .system))))
    }
    lock.lock()
    if finished {
      lock.unlock()
  ...
  nonisolated func cancel() {
    ...
    finish(.failure(ScreenCaptureError.captureFailed(AppText.value("scCaptureCancelled", language: .system))))
  }
